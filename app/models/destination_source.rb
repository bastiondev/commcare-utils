class DestinationSource < ApplicationRecord
  ...
  def sync_case case_data, owner={}, opened_by_user={}, closed_by_user={}
    ...
    (sensitive_fields || '').split(',').map(&:strip).each do |field|
      if properties[field].present?
        puts "Processing sensitive field: #{field}, value: #{properties[field]}"
  ...
  def sync_source
    ...
    if num_rows > 0
      Rails.logger.info "Synced #{num_rows} rows from destination source #{id}"
